			...
			<div class="row justify-content-center">
				...
				<div class="col-lg-3 col-md-6 col-sm-12 mb-4 mt-lg-0 pr-1 pl-1">
					 <div class="detail-img text-center dds">
						...
						?>
						<?php if($imagePath !=''){?>
						<img src="{{asset('front/profile/'.$imagePath)}}" alt="{{$imagePath}}" class="rounded w-100" style="max-height:210px;" {{$attr}}>
						<?php } else {?>
...
<!-- <div id="slideout" class="" style="right: -320px;" >
	<div id="slidecontent">
				  ...
				  @endphp
						...
						<?php if($trainerData->business_name != $bussiness_name){
							...
							if($page>0){
								...
								$keyword_search = isset($_GET['keyword_search']) ? $_GET['keyword_search'] : '';
							...
							if(Auth::guard('front_auth')->user()){
								...
								$favRatingClass = "favRatingCustomer";
							} else {
								$favRatingClass = "favRatingtrainer";
							}
							}else{
								$favRatingClass = "fav";
							}
						...
						<div class="add-fav" id="add_enable"> <a onclick="addrecommended('{{$trainerData->id}}')" href="javascript:void(0)"  class="add-favorite" ><i class="heart fa fa-regular fa-heart"></i></a><span class="tooltiptext">Add as Favorite</span> </div>
						...
						@else
						<div class="add-fav" id="add_enable" style="display:none;"> <a onclick="addrecommended('{{$trainerData->id}}')" href="javascript:void(0)"  class="add-favorite"><i class="heart fa fa-regular fa-heart"></i></a><span class="tooltiptext">Add as Favorite</span> </div>
			...
			<div class="row">
				...
				<nav>
					<div class="nav nav-tabs provider-nav" id="nav-tab" role="tablist">
						...
						<a class="nav-link" id="nav-network-tab" data-toggle="tab" href="#nav-network" role="tab" aria-controls="nav-contact" aria-selected="false">Network</a>
				...
				<div class="tab-content" id="nav-tabContent">
					<div class="tab-pane bg-color_sect fade show active" id="nav-businessbio" role="tabpanel" aria-labelledby="nav-businessbio-tab">
						<div class="row">
						<div class="col-md-8 col-sm-6 col-xs-6"><p class="text_style_tab">
						  ...
						  @endif 
							 ...
							 <div class="col-md-4 col-sm-6 col-xs-12">
								<div class="contact-box-inner mt-3 about-content contact_div">
									<span class="contact-title text-uppercase">Contact Info</span>
									@if($trainerData->trainer_email_contact == 0)
									<span>&nbsp;<button class="badge badge-success emailus_button" data-toggle="modal" data-target="#emailModal" style="border: none;padding: 6px;">Email Us</button> </span>
									@endif 
							 	<!-- <div id="slideout" class="" style="right: -320px;" >
								<div id="slidecontent">
								<div class="contact_div"> -->
								  <ul class="offshore_list">
											...
											<a target="_blank" href="http://maps.google.com/?q={{ $trainerData->address_1}}, {{$trainerData->city}}, {{$trainerData->state}}, {{$trainerData->zip_code}}">
														...
														@if($trainerData->address_1 != ''){{$trainerData->address_1}} @if($trainerData->address_2 != '') , @endif @endif
														@if($trainerData->address_2 != ''){{$trainerData->address_2}}@endif
		
														</span><?php echo ($trainerData->address_1 != '' || $trainerData->address_2 != '') ? "<br>": "" ?>
		
														<span>@if($trainerData->city != ''){{$trainerData->city}} @if($trainerData->state_code != ''),@endif @endif</span> 
														<span>@if($trainerData->state_code != ''){{$trainerData->state_code}} @if($trainerData->zip_code != ''),@endif @endif</span>
														<span>@if($trainerData->zip_code != ''){{$trainerData->zip_code}}@endif</span>
														<span>@if($trainerData->address1_virtual == 1)(Virtual Only)@endif</span>
															</a>
											</li>
											<li class="bold_text"><img src="{{asset('front/images/icon-phone.png')}}" class="tick_icon"><a href="tel:{{$trainerData->phone_number}}" class="phone_number">{{$trainerData->phone_number}}</a></li>
											@if(!empty($trainerData->website))
											...
											<li class="bold_text"><img src="{{asset('front/images/icon-time.png')}}" class="tick_icon">Hours of Operation<br><span class="bold dayName">Monday : </span>
												<?php
												if($trainerData->day1_check == 0){
												if($trainerData->day1 != ',' && !empty($trainerData->day1) && !$trainerData->day1 == NULL){ 
												$day1Result = explode(",", $trainerData->day1);
					...
					<div class="tab-pane bg-color_sect fade" id="nav-services" role="tabpanel" aria-labelledby="nav-services-tab">
						...
						<div class="blog_row row">
							...
							<div class="col-md-12">
								...
								<?php if(strlen($service->message) > 300){?>
								...
								<?php $provider_stripe_check = DB::select('select * from stripe_accounts where user_id="'.$service->trainer_id.'"');
								...
								@if($service->book_type == 1)
									@if(isset($service->price))
										@if(count($provider_stripe_check) != 0 || $service->price == 0)
										<a href="{{url('service-book-now')}}/{{base64_encode($service->id)}}" class="book_now">BOOK NOW</a>
										@endif
									@endif
								@else
									@if(isset($service->price))
										@if(count($provider_stripe_check) != 0 || $service->price == 0)
										<a href="{{url('service-book-now')}}/{{base64_encode($service->id)}}" class="book_now" id="request_book_now">REQUEST TO BOOK</a>
										@endif
									@endif
								@endif
 								@endif
								...
								?>
										@if(isset($service->price))
											@if(count($provider_stripe_check) != 0 || $service->price == 0)
											<a href="{{url('service-book-now')}}/{{base64_encode($service->id)}}"  class="book_now">REQUEST TO BOOK</a>
											@endif
										@endif
			  ...
			  <div class="row justify-content-center" style="padding: 10px;">
				...
				<div class="col-lg-4 col-sm-4">
				  <div class="form-group search-group">
					...
					<input type="hidden" value="{{ $resource_tab_search_name }}" id="resource_tab_search_name">
			...
			@csrf
			  ...
			  <div class="row justify-content-center">
				...
				<div class="col-lg-4 col-md-6 col-sm-12 mb-4 ">
				  ...
				  <a href="{{ url('/resource-details/')}}/{{ base64_encode($resource->id) }}"><div class="img_section"> <img src="{{ asset('front/images/resource/'.$resource->image_name) }}" class="img_responsive">
					
				  </div></a>
					@elseif($resource->format == 'Video')
					<a href="{{ url('/resource-details/')}}/{{ base64_encode($resource->id) }}">
						@if($resource->image_name != '')
		                  <div class="img_section"> <img src="{{ asset('front/images/resource/'.$resource->image_name) }}" class="img_responsive">
		                    @else
		                    <div class="img_section"> <img src="{{ asset('front/images/resource/details_default.png') }}" class="img_responsive">
		                    @endif
						<!-- <div class="img_section"> <img src="{{ asset('front/images/resource/'.$resource->image_name) }}" class="img_responsive"> -->
						<div class="overlay_img"></div>
                    	<div><img src="{{asset('front/images/play.png')}}" class="play_btn"></div>
				  </div></a>
				  ...
				  <div class="detail_img_tab">
					...
					<a href="{{ url('/resource-details/')}}/{{ base64_encode($resource->id) }}"><h3 class="img_tab_title">{{ $resource->title }}</h3></a>
				...
				?>
				  ...
				  <?php if($resource_count != 0){ if(count($resource_count_detail) !=0){ foreach ($resource_count_detail as $like) { if($like->likes == 1){?>
					...
					<div class="modal-dialog" role="document">
					  <div class="modal-content">
						...
						<?php 
							$resource_like_name = DB::table('resource_count')->where(["resource_id" => $resource->id, "likes" => 1])->orderBy('id', 'desc')->get();
					...
					<li class="list_style">
					  ...
					  <?php }
						...
						$resource_dislike_name = DB::table('resource_count')->where(["resource_id" => $resource->id, "dislike" => 1])->orderBy('id', 'desc')->get();
					...
					<div class="modal-dialog" role="document">
					  <div class="modal-content">
						...
						<div class="modal-body">
						  ...
						  <div class="p-2">
						  <form action="{{ route('resources-comment') }}" method="post" class="comment_submit">
						  
							<input type="hidden" name="resource_id" value="{{ $resource->id }}">
							<input type="hidden" name="resource_url" value="{{ url('/resource-details/')}}/{{ base64_encode($resource->id) }}">
							<input type="hidden" name="provider_name" value="{{ $trainerName->spot_description }}">
							<input type="hidden" name="provider_email" value="{{ $trainerName->email }}">
				  ...
				  <?php
                              ...
                              $image=urlencode('http://trainingblockdev.sgssys.info/public/front/images/resource/resource_image1629783647.jpg');
					...
					<li class="list_style">
					  ...
					  <div class="share-button sharer" style="display: block;">
						...
						<?php if($resource->format == 'Video'){ ?>
                          <a class="fbtn share facebook" href="https://www.facebook.com/sharer/sharer.php?u={{ $resource->format_name }}&p[title]={{ $resource->title }}" target="_blank" ><i class="fa fa-brands fa-facebook"></i></a> 
                          <?php } else { ?>
                            <a class="fbtn share facebook" href="https://www.facebook.com/sharer/sharer.php?u={{ asset('front/images/resource/'.$resource->image_name) }}&p[title]={{ $resource->title }}" target="_blank" ><i class="fa fa-brands fa-facebook"></i></a> 
                          <?php } ?>
                          <?php if($resource->format == 'Video'){ ?>
                          <a class="fbtn share twitter" href="https://twitter.com/intent/tweet?text={{ $resource->title }}&url={{ $resource->format_name }}" target="_blank" ><i class="fa fa-brands fa-twitter"></i></a>
                          <?php } else { ?>
                            <a class="fbtn share twitter" href="https://twitter.com/intent/tweet?text={{ $resource->title }}&url={{ asset('front/images/resource/'.$resource->image_name) }}" target="_blank" ><i class="fa fa-brands fa-twitter"></i></a>
                            <?php } ?>
                          <a class="fbtn share envelope" href="mailto:?subject={{ $resource->title }}&amp;body=<?php echo $currentURL;?>" target="_blank" ><i class="fa fa-envelope"></i></a>
					...
					<div class="modal-dialog" role="document">
					  <div class="modal-content">
						...
						<?php 
							...
							$profile_img = DB::table('front_users')->where(["id" => $name->user_id])->first();
						  ?>
						  <a href="{{ url('providers/'.$trainerName->spot_description)}}">
						  <img src="@if(isset($profile_img->photo) && !empty($profile_img->photo)) {{ asset('front/profile/'.$profile_img->photo)}} @else {{ asset('front/images/details_default.png')}}  @endif" class="comments_profile">
						  <span class="likes_name">{{ $name->name }}</span>
						  <span class="short_text">{{ \Carbon\Carbon::parse($name->created_at)->diffForHumans() }}</span></a><br><br>
						  <?php }?>
                      ...
                      <div class="modal-dialog modal-sm" role="document">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-body" style="height: 90px;text-align: center;overflow-y:hidden;">
                            <p>you must <a href="{{ url('providers/'.$trainerName->spot_description)}}" style="text-decoration: underline;color: #00ab91;">login</a></p>
                          </div>
...
<!-- Resource Modal -->
					...
					<!-- Review and network Tab -->
					<div class="tab-pane bg-color_sect fade" id="nav-network" role="tabpanel" aria-labelledby="nav-network-tab">
						...
						<div class="row">
							<div class="col-md-12 col-sm-12 col-xs-12">
								...
								<div class="contact-box-inner mt-3 about-content contact_div">
									...
									@if($Trainer->users->status == 'active')
									@if($Trainer->users->is_subscription == '0' || $Trainer->users->is_payment == '1')
									<div class="media mb-2 col-md-4" id="addrecommended_id_{{$Trainer->users->id}}" >
												...
												<div class="media-body" style="padding-right: 18px;">
													...
													<img onclick="removerecommended({{$Trainer->users->id}})" class="float-right" style="cursor: pointer;margin: 10px 15px;position: absolute;top: -6px;right: 0;" src="{{asset('images/delete-icon.png')}}" alt="Rating" />
													@endif
												</div>
											</div>
											@endif
				...
				@foreach($recommended_authlete as $recomm)
							<div class="col-xl-6 mb-2">  
								<div class="user-info d-flex">
									<img class="user-image " style="height: 55px; width: 55px;object-fit: cover;border-radius: 50%;" src="@if(isset($recomm->customer->photo) && !empty($recomm->customer->photo) && file_exists(public_path('front/profile/'.$recomm->customer->photo))) {{ asset('front/profile/'.$recomm->customer->photo)}} @else {{ asset('front/images/details_default.png')}}  @endif">
...
<div class="modal fade" id="emailModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-lg" role="document">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-header">
                            <h4 class="modal-title w-100" id="myModalLabel">Email Us</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                            </button>
                          </div>
						  ...
						  <div class="alert alert-success" id="email_success" role="alert" style="display:none"></div>
							...
							<div class="row">
                                    <div class="col-md-12">
										<input type="hidden" name="trainerFirstName" id="trainerFirstName" value="{{$trainerData->first_name}}">
										<input type="hidden" name="trainerLastName" id="trainerLastName" value="{{$trainerData->last_name}}">
										<input type="hidden" name="receiverId" id="receiverId" value="{{$trainerData->id}}">
										<input type="hidden" name="trainer_email" id="trainer_email" value="{{$trainerData->email}}">
										<div class="row">
										<div class="col-md-6">
									...
									<div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="subject" id="subject" placeholder="Enter Subject" maxlength="150">
                                    </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea rows="" class="form-control" name="email_body" id="email_body" cols="10" maxlength="1000"></textarea>
                                    </div>
                                    </div>
									...
									<div class="col-md-12">
									<button type="submit" id="emailSubmit" class="btn btn-info btn-lg mb-3 float-right">Send</button>
...
@section('pagescript')
<style> 
#emailUsForm label.error, .required {
    color: #ff0023;
    font-weight: 100;
}
/* .detail-img img {
...
var resource_tab_search_name = $('#resource_tab_search_name').val();
if(resource_tab_search_name == '10'){
	$('.nav-tabs a[href="#nav-resources"]').tab('show');
}
//alert($('#resource_tab_search_name').val());
...
$('.stopModal').on('hidden.bs.modal', function (e) {

  $(this).filter('iframe').modal('hide');
//   $('modalID video').attr("src", jQuery(".stopModal video").attr("src"));
});
...
$('.nav a:not(".external")').click(function (e) {
 	$('#resource_tab_search_name').val('');
 	/*hrefurl=$(location).attr("href");
 	var str = hrefurl.substr(hrefurl.lastIndexOf('#') + 1);
    return hrefurl.replace( new RegExp(str), '' );*/
})
...
$(function() {
  ...
  $("form[name='emailUsForm']").validate({
	...
	// if($('#emailSubscribe').is(':checked')){
	// 	join_newsletter();
	// }
	// if($('#emailRegister').is(':checked')){
	// 	checkEmail();
	// }
	// else {
	$('#emailSubmit').attr('disabled','disabled');
	$("#emailSubmit").html('Sending...');
	var trainerFirstName = $('#trainerFirstName').val() != '' ? $('#trainerFirstName').val() : '';
	var trainerLastName = $('#trainerLastName').val() != '' ? $('#trainerLastName').val() : '';
	var toEmail = $('#trainer_email').val();
	var emailBody = $('#email_body').val();
	var receiverId = $('#receiverId').val();
	// register_athelete();
	$.ajax({
		url: "{{ route('trainer.provider_emailus') }}",
		type: "POST",
		data : $('form').serialize(),
		headers: {
					'X-CSRF-TOKEN': '{{ csrf_token() }}'
		...
		success: function(response){
			...
			// $('#email_success').html('Email send successfully');
			$('#emailUsForm')[0].reset();
			grecaptcha.reset();
			$('#emailSubmit').removeAttr('disabled');
			$("#emailSubmit").html('Send');
			$('.password_section').css('display', 'none');
			// $("#email_success").fadeTo(3000, 500).slideUp(500, function(){
    		// $("#email_success").slideUp(500);
			// });
